...
def runPremadeTestCases():
   ...
   nonHomographStrings = [f"{cwd}..\\week04\\test.txt", f"{cwd}./NtaHomaGram.txt", f"{cwd}test", 
   f"{cwd}..\\..\\..\\..\\home\\cs301\\week05\\test.txt"]
   ...
   for i in range(0, len(homographStrings)):
      ...
      result = runTest(homographStrings[i], homographStrings[len(homographStrings)-1-i])
      print(homographStrings[i])
      print(homographStrings[len(homographStrings)-1-i])
      if result:
         print("These Two Paths Are Homographs\n")
      else:
         print("These Two Paths Are NOT Homographs\n")
